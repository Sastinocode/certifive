...
app.post("/api/auth/register", registerRateLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [user] = await db.insert(users).values({
      ...
      // Prueba gratuita de 14 días. Tras expirar, subscription-guard bloquea
      // el acceso hasta que el usuario seleccione un plan de pago.
      subscriptionStatus: "trialing",
      subscriptionCurrentPeriodEnd: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000), // 14 días
    }).returning();
